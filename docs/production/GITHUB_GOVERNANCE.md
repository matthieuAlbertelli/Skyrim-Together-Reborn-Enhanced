...
The `STRE road to v1.0.0` Project is the live operational board. Its minimum workflow is `Todo`, `In progress`, `Blocked`, and `Done`; backlog grouping may be added only if it remains operationally useful. Use built-in issue fields plus Milestone, labels and parent/sub-issue relationships instead of duplicating priority, area or target version in custom fields.
...
PRs target `main`, use focused commits and remain unmerged until required review and checks pass. Structural decisions use an ADR; feature-local behavior and protocols remain in the feature's canonical documentation directory.
...
4. Create the `STRE road to v1.0.0` Project, link it to the repository, configure the minimum workflow, and add the accepted v1 issues.
...
6. Protect `main` with a ruleset requiring pull requests, review and the relevant successful checks. Do not enable automatic merge as part of this setup.
